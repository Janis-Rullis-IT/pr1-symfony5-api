...
class ProductValidator
{
    ...
    public function validate(array $data, int $id_user): void
    {
        ...
        foreach ($data as $key => $value) {
            if (Product::TYPE === $key && !$this->productTypeValidator->validate($value)) {
                $this->errorsLoader->load(Product::TYPE, Product::INVALID_TYPE, $this->errors);
            } elseif (Product::TITLE === $key && !$this->titleValidator->validate($value)) {
                $this->errorsLoader->load(Product::TITLE, Product::INVALID_TITLE, $this->errors);
            } elseif (Product::SKU === $key && !$this->skuValidator->validate($value, $id_user)) {
                $this->errorsLoader->load(Product::SKU, Product::INVALID_SKU, $this->errors);
            } elseif (Product::COST === $key && (!is_int($value) || $value < 1)) {
                $this->errorsLoader->load(Product::COST, Product::INVALID_COST, $this->errors);
            }
        }

        if (!empty($this->errors)) {
            if (array_key_exists(Product::SKU, $this->errors) && in_array(Product::INVALID_SKU, $this->errors[Product::SKU]) && 1 == count($this->errors)) {
